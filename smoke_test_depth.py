...
Computational-feasibility smoke test for the PHN circuit-DEPTH-scaling
experiment (RESEARCH QUESTION: at a fixed qubit count of 10 -- already
established as computationally feasible on the Tesla T4 by
smoke_test_qubits.py -- does increasing VQC circuit depth beyond the
VQC_DEPTH=2 baseline remain computationally tractable?).

This script is a STANDALONE SIBLING to smoke_test_qubits.py. It does
NOT modify, import internals from, or otherwise touch that script or
the existing training pipeline. It follows the same measurement
protocol and the same non-negotiable guardrails, applied to a
different sweep axis (VQC_DEPTH instead of VQC_NUM_QUBITS).
...
answer: "at 10 qubits, is each candidate circuit depth computationally
tractable on the actual GPU (Tesla T4), before committing to an
overnight training run?"

For each depth in {2, 3, 4}, with VQC_NUM_QUBITS fixed at 10, this
script:

  1. Constructs a fresh `proposed_phn` model
     (get_model("proposed_phn", ...) -> ProposedModel(use_quantum_branch=True))
     with VQC_NUM_QUBITS=10 and VQC_DEPTH overridden via config for
     that run only. All other config values (VQC_SIMULATOR,
     VQC_DIFF_METHOD, dataset, split, optimizer, lr, weight decay,
     real training batch size, etc.) are read from the existing
     config unchanged.
  ...
  5. Prints a summary table across all depths and an order-of-
     magnitude full-training runtime ESTIMATE based on the ACTUAL step/
     epoch counts in configs/config.py (NUM_EPOCHS, BATCH_SIZE,
     EARLY_STOPPING_PATIENCE), scaled from this run's measured
     per-step time -- NOT assumed to scale linearly with depth.

IMPORTANT: this script uses a FRESH SUBPROCESS per depth (where
practical) so that PennyLane device/QNode state, CUDA memory, and
Python-level caching from one depth cannot leak into or bias the
...
Neither the 2-qubit baseline nor the 10-qubit/depth-2 feasibility
result is re-measured here as a performance claim -- this script only
benchmarks computational feasibility at the NEW candidate depths: 3
and 4 (depth 2 is included as the in-sweep reference point so all
three depths are measured under identical conditions in the same
table).

SCOPE / NON-GOALS (explicit)
-----------------------------
  - Does NOT train the model.
  - Does NOT evaluate forecasting performance.
  - Does NOT modify configs/config.py, models/model_factory.py, or
    any other pipeline file.
  - Does NOT modify or import smoke_test_qubits.py.
  - Does NOT touch the separate PHNModel / phn_model.py architecture.

USAGE
-----
    python smoke_test_depth.py

Optional flags:
    python smoke_test_depth.py --depths 2 3 4          # default
    python smoke_test_depth.py --depths 3 4             # skip baseline
    python smoke_test_depth.py --num-qubits 10           # default; fixed
                                                          # per the research
                                                          # question, but
                                                          # overridable for
                                                          # debugging
    python smoke_test_depth.py --batch-size 4             # default is 4
    python smoke_test_depth.py --opt-steps 5               # 0 disables the
                                                            # preliminary
                                                            # optimization
                                                            # signal
    python smoke_test_depth.py --no-subprocess              # force in-process
                                                            # (single Python
                                                            # process) mode
"""
...
# CLI entry points and with smoke_test_qubits.py. It imports
...
# =============================================================================
# Single-depth benchmark (runs in its own process when invoked via
# --_internal-single-run, or in-process as a fallback)
# =============================================================================

def _run_single_depth_benchmark(
    num_qubits: int,
    vqc_depth: int,
    batch_size: int,
    opt_steps: int,
    ...
    result = DepthBenchmarkResult(num_qubits=num_qubits, vqc_depth=vqc_depth)
    ...
    try:
        ...
        # ---- Override ONLY VQC_NUM_QUBITS and VQC_DEPTH for this run.
        # Everything else (VQC_SIMULATOR, VQC_DIFF_METHOD, dataset,
        ...
        # Best-effort depth confirmation. The attribute name for depth
        # is not guaranteed across VQC branch implementations, so this
        # check is advisory only: if a recognizable attribute exists
        # and disagrees with the requested depth, that's a hard
        # failure (config override silently ignored); if no such
        # attribute exists, we proceed without asserting on it rather
        # than guessing at an attribute name that may not apply here.
        for attr_name in ("depth", "num_layers", "vqc_depth", "n_layers"):
            if hasattr(model.vqc_branch, attr_name):
                observed_depth = getattr(model.vqc_branch, attr_name)
                if observed_depth != vqc_depth:
                    raise RuntimeError(
                        f"model.vqc_branch.{attr_name}={observed_depth} "
                        f"does not match requested vqc_depth={vqc_depth} -- "
                        "config override did not take effect as expected."
                    )
                break
        ...
        #   - the deeper circuit is numerically trainable at all,
        ...
        #     prediction)
        #   - config.BATCH_SIZE (the REAL training batch size) -- NOTE
        #     this differs from this smoke test's small batch_size
        #     (default 4), used here only for feasibility probing
        #     given deeper-circuit statevector cost.
        #   - config.NUM_RUNS (repeated runs per the existing
        #     multi-run protocol)
        #
        ...
        #     PER-EPOCH-PER-STEP time and leaves total-epoch scaling to
        #     the reader, rather than fabricating a training-set size;
        #   - step time at BATCH_SIZE is NOT assumed to equal step
        #     time at this smoke test's batch_size -- VQC execution is
        #     CPU-bound and per-sample serial in several PennyLane
        #     execution paths, so this script reports the measured
        #     per-sample step time (step_ms / batch_size) and
        #     extrapolates LINEARLY IN BATCH SIZE ONLY as a labeled
        #     approximation, while explicitly NOT assuming the
        #     per-depth scaling itself is linear (that is the entire
...
# =============================================================================
# Subprocess wrapper (isolation between depths)
# =============================================================================

def _run_in_subprocess(
    num_qubits: int, vqc_depth: int, batch_size: int, opt_steps: int, seed: int
) -> DepthBenchmarkResult:
    """
    Runs `_run_single_depth_benchmark` in a fresh `python` subprocess
    ...
    cmd = [
        ...
        "--_internal-vqc-depth", str(vqc_depth),
    ...
    if proc.returncode != 0:
        result = DepthBenchmarkResult(
            num_qubits=num_qubits, vqc_depth=vqc_depth, status="FAIL"
        )
    ...
    for line in proc.stdout.splitlines():
        if line.startswith("RESULT_JSON:"):
            ...
            return DepthBenchmarkResult(**payload)
...
def print_timing_table(results: list[DepthBenchmarkResult]) -> None:
    print()
    print("TIMING / PARAMETER TABLE")
    print(
        f"{'Depth':>6} | {'Params':>10} | {'VQC Params':>10} | "
        f"{'Forward ms':>12} | {'Backward ms':>12} | {'Step ms':>10}"
    )
    print("-" * 78)
    for r in results:
        if r.status != "PASS":
            print(f"{r.vqc_depth:>6} | {'FAILED':>10} | {'-':>10} | "
                  f"{'-':>12} | {'-':>12} | {'-':>10}")
            continue
        ...
        print(
            f"{r.vqc_depth:>6} | {r.total_param_count:>10} | "
            f"{r.vqc_param_count:>10} | {fwd:>12} | {bwd:>12} | {step:>10}"
        )


def print_status_table(results: list[DepthBenchmarkResult]) -> None:
    print()
    print("STATUS TABLE")
    print(
        f"{'Depth':>6} | {'GPU Mem (MB)':>13} | {'Output Shape':>14} | "
    ...
    for r in results:
        ...
        print(
            f"{r.vqc_depth:>6} | {gpu_mem:>13} | {out_shape:>14} | "
...
def print_runtime_estimates(
    results: list[DepthBenchmarkResult], config
) -> None:
    print()
    ...
    print(
        ...
        "time x real BATCH_SIZE), NOT linear scaling in circuit depth -- "
        "depth-vs-runtime scaling is exactly what this table is meant to "
        "reveal, not assume. Uses the pipeline's ACTUAL "
    ...
    print()
    print(f"{'Depth':>6} | {'Est. ms/step @ real batch size':>32} | {'Status':>8}")
    print("-" * 78)
    for r in results:
        if r.status != "PASS":
            print(f"{r.vqc_depth:>6} | {'-':>32} | {r.status:>8}")
            continue
        est = r.real_batch_step_ms_estimate
        est_str = f"{est:.1f} ms" if est is not None else "N/A"
        print(f"{r.vqc_depth:>6} | {est_str:>32} | {r.status:>8}")


def print_preliminary_optimization_signal(
    results: list[DepthBenchmarkResult],
) -> None:
    ...
    for r in results:
        ...
        print(
            f"  vqc_depth={r.vqc_depth} (num_qubits={r.num_qubits}): "
            f"losses=[{losses_str}] "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        "circuit-depth scaling at a fixed 10-qubit count (candidates "
        "depth 2-4, against the existing VQC_DEPTH=2 baseline)."
    )
    parser.add_argument(
        "--depths", type=int, nargs="+", default=CANDIDATE_DEPTHS,
        help="Circuit depths to benchmark (default: 2 3 4).",
    )
    parser.add_argument(
        "--num-qubits", type=int, default=FIXED_NUM_QUBITS,
        help="Qubit count to fix for this sweep (default: 10, per the "
        "research question -- already established as feasible by "
        "smoke_test_qubits.py). Overridable for debugging only.",
    )
    parser.add_argument(
        ...
        "deliberately small because deeper 10-qubit circuits may be "
        "expensive.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "instead of one fresh subprocess per depth. Subprocess "
        "isolation is preferred (avoids CUDA-memory/QNode-state leakage "
        "between depths) but this flag allows a fallback.",
    )
    parser.add_argument(
        ...
        "used for every depth so any behavior difference between them "
        "is not an artifact of a different starting point.",
    )

    # Internal flags used to re-invoke this script as a subprocess for a
    # single depth. Not intended for direct use.
    ...
    parser.add_argument("--_internal-vqc-depth", type=int, default=None, help=argparse.SUPPRESS)
    ...
    if args._internal_single_run:
        result = _run_single_depth_benchmark(
            num_qubits=args._internal_num_qubits,
            vqc_depth=args._internal_vqc_depth,
    ...
    print(f"Fixed qubit count          : {num_qubits}")
    print(f"Benchmarking circuit depths: {args.depths}")
    ...
    print(f"Isolation mode             : "
          f"{'in-process (forced)' if args.no_subprocess else 'subprocess per depth'}")
    print()

    results: list[DepthBenchmarkResult] = []

    for depth in args.depths:
        print(f"--- num_qubits={num_qubits} vqc_depth={depth} " + "-" * 20)
        if args.no_subprocess:
            ...
            try:
                ...
                print(
                    ...
                    "falling back to in-process for this depth."
                )
                gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
    ...
    print()
    print("=" * 78)
    print(f"SMOKE TEST COMPLETE (num_qubits={num_qubits}, depths={args.depths}). "
          "No training was started.")
    print("Review PASS/FAIL, memory, and timing above before deciding")
    print("which depth configuration(s), if any, are worth training")
